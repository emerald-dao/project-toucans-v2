<script lang="ts">
	...
	import { createNFLFilters } from '$lib/features/nft-treasury/functions/NFLAllDay/NFLAllDayFilters';
	import { filterNFLContent } from '$lib/features/nft-treasury/functions/NFLAllDay/NFLAllDayContentFilter';
	import type { NFLFilter } from '$lib/features/nft-treasury/types/NFLAllDay/nfl-filter-interface';
	...
	onMount(() => {
		filters = createNFLFilters(activeFilters);
	});

	$: terms = createSearchTerms(nfts);
	...
	function createCombinedSearchStore() {
		...
		const combinedSearchStore = derived(storeList, (stores) => {
			...
			filteredContent = filterNFLContent(filters, result, activeFilters);
		});
	...
	$: if (
		...
		filteredContent = filterNFLContent(filters, nfts, activeFilters);
	} else {

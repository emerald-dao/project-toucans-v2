...
access(all) contract FiatToken: FungibleToken {
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        /// Get all the Metadata Views implemented by FiatToken
        ///
        ...
        /// Get a Metadata View from FiatToken
        ///
    ...
    access(all) fun resolveContractView(resourceType: Type?, viewType: Type): AnyStruct? {
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                let vaultRef = FiatToken.account.storage.borrow<auth(FungibleToken.Withdraw) &FiatToken.Vault>(from: /storage/FiatTokenVault)
			        ?? panic("Could not borrow reference to the contract's Vault!")
                return FungibleTokenMetadataViews.FTVaultData(
                    storagePath: /storage/FiatTokenVault,
                    receiverPath: /public/FiatTokenReceiver,
                    metadataPath: /public/FiatTokenBalance,
            ...
            case Type<FungibleTokenMetadataViews.TotalSupply>():
                return FungibleTokenMetadataViews.TotalSupply(totalSupply: FiatToken.totalSupply)
        }
        return nil
    }
    ...
    access(all) fun getLogoURI(): String {
        return FiatToken.account.storage.copy<String>(from: /storage/FiatTokenLogoURI) ?? ""
    }
    ...
    init(adminAccount: auth(Storage, Capabilities) &Account) {
        ...
        adminAccount.storage.save(<-vault, to: /storage/FiatTokenVault)

        // Create a public capability to the stored Vault that only exposes
        // the `deposit` method through the `Receiver` interface
        //
        let receiverCapability = adminAccount.capabilities.storage.issue<&FiatToken.Vault>(/storage/FiatTokenVault)
        adminAccount.capabilities.publish(receiverCapability, at: /public/FiatTokenReceiver)

        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        //
        let balanceCapability = adminAccount.capabilities.storage.issue<&FiatToken.Vault>(/storage/FiatTokenVault)
        adminAccount.capabilities.publish(balanceCapability, at: /public/FiatTokenBalance)

        let admin <- create Administrator()
        adminAccount.storage.save(<-admin, to: /storage/FiatTokenAdmin)

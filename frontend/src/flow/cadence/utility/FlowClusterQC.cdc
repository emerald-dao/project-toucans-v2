...
access(all) contract FlowClusterQC {
    ...
    access(all) var clusters: [Cluster]
    ...
    access(all) var voterClaimed: {String: Bool}
    ...
    access(all) struct Cluster {
        ...
        access(all) fun voteThreshold(): UInt64 {
        ...
        access(all) fun isComplete(): String? {
    ...
    access(all) fun voterIsRegistered(_ nodeID: String): Bool {
    ...
    access(all) fun getClusters(): [Cluster] {
        return self.clusters
    }

    /// Returns true if we have collected enough votes for all clusters.
    access(all) fun votingCompleted(): Bool {

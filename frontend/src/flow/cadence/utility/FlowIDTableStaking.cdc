...
access(all) contract FlowIDTableStaking {
    ...
    access(all) var nodeDelegatingRewardCut: UFix64
    ...
    access(all) resource NodeRecord {
        ...
        access(all) fun nodeFullCommittedBalance(): UFix64 {
        ...
        access(all) fun borrowDelegatorRecord(_ delegatorID: UInt32): &DelegatorRecord {
            pre {
        ...
        access(all) fun setDelegator(delegatorID: UInt32, delegator: @DelegatorRecord) {
    ...
    access(all) fun borrowNodeRecord(_ nodeID: String): &NodeRecord {
        pre {
    ...
    access(all) fun borrowFeesAdmin(): &FlowFees.Administrator {
    ...
    access(all) fun updateClaimed(path: StoragePath, _ key: String, claimed: Bool) {
    ...
    access(all) fun isValidNodeID(_ input: String): Bool {
    ...
    access(all) fun isGreaterThanMinimumForRole(numTokens: UFix64, role: UInt8): Bool {
    ...
    access(all) fun getClaimed(path: StoragePath, key: String): Bool {
		let claimedDictionary = self.account.borrow<&{String: Bool}>(from: path)
            ?? panic("Invalid path for dictionary")
    ...
    access(all) fun getMinimumStakeRequirements(): {UInt8: UFix64} {
    ...
    access(all) fun getTotalTokensStakedByNodeType(): {UInt8: UFix64} {
